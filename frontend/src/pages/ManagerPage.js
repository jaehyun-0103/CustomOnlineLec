...
const Manage = () => {
  ...
  const fetchUserData = () => {
    ...
    axios
      .get("http://43.201.10.158:8080/admin/user/list", {
        headers: {
  ...
  const fetchQnaData = () => {
    axios
      .get("http://43.201.10.158:8080/qna/list", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  const handleDeleteVideoButtonClick = (videoId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`http://43.201.10.158:8080/admin/${videoId}`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          })
  ...
  const handleDeleteQnaButtonClick = (qnaId) => {
    Swal.fire({
      ...
      axios
        .delete(`http://43.201.10.158:8080/qna/${qnaId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
