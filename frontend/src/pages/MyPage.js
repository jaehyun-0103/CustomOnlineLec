...
const MyPage = () => {
  ...
  const handleSaveClick = () => {
    axios
      .patch(
        `http://43.201.10.158:8080/mypage/update/${username}`,
        {
  ...
  const sendS3UrlToServer = (s3Url) => {
    axios
      .patch(
        `http://43.201.10.158:8080/mypage/update/profile/${username}`,
        { ProfileS3Path: s3Url },
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  useEffect(() => {
    ...
    async function fetchProfileInfo() {
      try {
        const response = await axios.get(`http://43.201.10.158:8080/mypage/${username}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
    ...
    async function fetchVideos() {
      try {
        const response = await axios.get("http://43.201.10.158:8080/videos/list", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const handleWithdrawalClick = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`http://43.201.10.158:8080/mypage/delete/${username}`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          })
